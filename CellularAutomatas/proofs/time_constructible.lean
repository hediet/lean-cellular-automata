...
Given a CA C and a time-constructible function t, we can create a CA that:
1. Runs C in parallel with timer
2. Latches C's output at position 0 when timer fires
3. Preserves the latched value afterward

This solves the speedup problem: even if we continue computing past t(n),
we can report the acceptance value from exactly time t(n).
-/

/-- State for a CA with latched output at a specific time -/
structure LatchedState (Q : Type) (T : Type) where
  /-- Original CA state -/
  ca_state : Q
  /-- Timer CA state -/
  timer_state : T
  /-- Latched acceptance value (Some once timer fires) -/
  latched : Option Bool
deriving DecidableEq, Inhabited, Fintype

instance LatchedState.alphabet (Q T : Type) [Alphabet Q] [Alphabet T] :
    Alphabet (LatchedState Q T) where

/-- Product CA that runs original CA and timer in parallel, latching at signal -/
def latchedCA {α β : Type} [Alphabet α] [Alphabet β]
    (C : CellAutomaton α？ β) (t : ℕ → ℕ) (tc : TimeConstructible t)
    (accept : β → Bool) : CellAutomaton α？ Bool where
  Q := LatchedState C.Q tc.timer.Q
  δ := fun left mid right =>
    ...
    let new_latched :=
      ...
      else if timer_signal then some (accept (C.project ca_next))
      else none
    ⟨ca_next, timer_next, new_latched⟩
  embed := fun a =>
    ⟨C.embed a, tc.timer.embed (a.map fun _ => ()), none⟩
  project := fun s => s.latched.getD false

namespace LatchedCA

variable {α β : Type} [Alphabet α] [Alphabet β]
variable (C : CellAutomaton α？ β) (t : ℕ → ℕ) (tc : TimeConstructible t) (accept : β → Bool)
...
lemma ca_component_sync (w : Word α) (k : ℕ) (p : ℤ) :
    ((latchedCA C t tc accept).nextt ⦋w⦌ k p).ca_state = C.nextt ⦋w⦌ k p := by
  -- Induction showing CA component evolves independently
  sorry

/-- Helper: the timer component of latchedCA evolves as tc.timer would.

    Note: tc.timer uses unitWord n as input, while latchedCA uses the original word.
    The timer only cares about word length for border detection. -/
lemma timer_component_sync (w : Word α) (k : ℕ) (p : ℤ) :
    ((latchedCA C t tc accept).nextt ⦋w⦌ k p).timer_state =
    tc.timer.nextt ⦋unitWord w.length⦌ k p := by
  -- Induction showing timer component evolves independently
  sorry

/-- At time t(n), the latch is triggered with the correct value.
...
lemma latch_triggered_at_t (w : Word α) :
    ((latchedCA C t tc accept).nextt ⦋w⦌ (t w.length) 0).latched =
    some (accept (C.project (C.nextt ⦋w⦌ (t w.length) 0))) := by
  sorry

/-- Once latched, the value persists.

    By construction: if mid.latched.isSome then new_latched = mid.latched -/
lemma latch_persists (w : Word α) (k j : ℕ) (hk : k = t w.length) (hj : j ≥ k) :
    ((latchedCA C t tc accept).nextt ⦋w⦌ j 0).latched =
    ((latchedCA C t tc accept).nextt ⦋w⦌ k 0).latched := by
  sorry

end LatchedCA

/-- The latched CA correctly captures the acceptance value at time t(n).

    **Proof outline**:
    1. **Latch triggers at t(n)**: At time t(n), tc.timer signals (by tc.signal_at_t),
       so the latch captures `accept (C.project (C.nextt ⦋w⦌ t(n) 0))`.
    2. **Latch persists**: Once latched.isSome, it never changes (by construction).
    3. **No premature latch**: For k < t(n), tc.timer doesn't signal (by tc.no_signal_before),
       so latched remains none.

    **Dependencies**: latch_triggered_at_t, latch_persists -/
theorem latchedCA_correct {α β : Type} [Alphabet α] [Alphabet β]
    (C : CellAutomaton α？ β) (t : ℕ → ℕ) (tc : TimeConstructible t)
    (accept : β → Bool) (w : Word α) (k : ℕ) (hk : k ≥ t w.length) :
    (latchedCA C t tc accept).project ((latchedCA C t tc accept).nextt ⦋w⦌ k 0) =
    accept (C.project (C.nextt ⦋w⦌ (t w.length) 0)) := by
  -- The latch was triggered at time t(n) and persists
  -- Using the helper lemmas, this follows directly
  have h1 := LatchedCA.latch_triggered_at_t C t tc accept w
  have h2 := LatchedCA.latch_persists C t tc accept w (t w.length) k rfl hk
  -- The goal follows from h1, h2, and the definition of project
  -- project s = s.latched.getD false, and latched = some (accept ...)
  sorry
...
theorem time_extension {α : Type} [Alphabet α]
  ...
  -- Use latchedCA to run C and timer in parallel.
  -- The timer fires at t(n), latching C's acceptance value.
  -- At time t'(n) ≥ t(n), the latched value is still available.
  sorry
