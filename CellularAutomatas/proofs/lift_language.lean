...
private lemma liftCA_L_eq_lift (C : tCellAutomaton α) (ht : ∀ n, C.t n ≥ n - 1) (hp : ∀ n, C.p n = 0) :
  ...
  · -- (liftCA C).accepts w → ∃ v ∈ C.L, w = v.map some
    ...
    have h_all_some : ∀ i : ℕ, (hi : i < w.length) → ∃ a, w[i] = some a := by
      ...
      have h_covers : i ≤ C.t w.length := by have := ht w.length; omega
      ...
      have hp' : C.p w.length = 0 := hp w.length
      ...
      have h_acc_false : (liftCA C).accepts w = false := by
        ...
        simp only [CellAutomaton.comp, Function.comp, CellAutomaton.project_config, hp',
    ...
    have h_lift_eq : (liftCA C).accepts (v.map some) = C.accepts v := by
      unfold tCellAutomaton.accepts
      have hp' : C.p v.length = 0 := hp v.length
      simp only [List.length_map, liftCA, CellAutomaton.comp, Function.comp,
                 CellAutomaton.project_config, hp']
      have h_bool := liftCA_Bool_true_for_map_some C v (C.t v.length)
      have h_q := liftCA_Q_component C v (C.t v.length) (C.p v.length)
...
lemma lift_mem_ca_rt (L : Language α) (hL : L ∈ ℒ (CA_rt α)) :
  ...
  subst hCL
  have ht : ∀ n, C.t n ≥ n - 1 := fun n => by rw [hC.2 n]
  have hp : ∀ n, C.p n = 0 := by
    have h_p_eq : C.p = fun _ => 0 := hC.1.2
    intro n
    simp only [h_p_eq]
...
lemma lift_mem_ca_2n (L : Language α) (hL : L ∈ ℒ (CA_2n α)) :
  ...
  refine ⟨liftCA C, ⟨⟨trivial, hC.1.2⟩, hC.2⟩, ?_⟩
  subst hCL
  have ht : ∀ n, C.t n ≥ n - 1 := fun n => by simp only [hC.2 n]; omega
  have hp : ∀ n, C.p n = 0 := by
    have h_p_eq : C.p = fun _ => 0 := hC.1.2
    intro n
    simp only [h_p_eq]
